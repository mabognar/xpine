...
fn draw_email_list(stdout: &mut std::io::Stdout, app: &App, cols: u16, rows: u16, theme_provider: &Editor) -> io::Result<()> {
    ...
    if app.total_messages == 0 || app.page_emails.is_empty() {
        ...
        queue!(stdout,
            cursor::MoveTo(cols.saturating_sub(no_msg.len() as u16), 0),
            SetForegroundColor(colors.accent),
            Print(no_msg)
        )?;
    } else {
        ...
        let total_len = 8 + x_str.len() + 4 + y_str.len();
        ...
        queue!(stdout,
            ...
            SetForegroundColor(colors.fg), Print(&y_str)
        )?;
    }
    ...
    for (i, email) in app.page_emails.iter().enumerate() {
        ...
        // NEW: Attachment character
        let attach_char = if email.has_attachments { "&" } else { " " };
        ...
        queue!(
            ...
            SetForegroundColor(status_color), Print(status_char),

            // Replaced the `Print(" ")` with the attachment indicator logic directly!
            SetForegroundColor(Color::Cyan), Print(attach_char),
...
//                     Self::draw_menu_line(
//                         &mut stdout, rows - 2, cols, col_width,
//                         &[("<", " Back"), ("R", " Reply"), ("  Y", " Prev Pg"), ("^P", " Prv Msg"), ("D", " Delete"), ("O", " Other (1/2)")],
//                         ui_bg, menu_key_fg, menu_text_fg)?;
//                     Self::draw_menu_line(
//                         &mut stdout, rows - 1, cols, col_width,
//                         &[("B", " Browser"), ("F", " Forward"), ("Spc", " Next Pg"), ("^N", " Nxt Msg"), ("A", " Add Addr"), ("", "")],
//                         ui_bg, menu_key_fg, menu_text_fg)?;
//                 } else {
//                     Self::draw_menu_line(
//                         &mut stdout, rows - 2, cols, col_width,
//                         &[("P", " Scroll Up"), ("#", " Open Att"), ("Alt+#", " Save Att"), ("Alt+0", " Save All"), ("", ""), ("O", " Other (2/2)")],
//                         ui_bg, menu_key_fg, menu_text_fg)?;
//                     Self::draw_menu_line(
//                         &mut stdout, rows - 1, cols, col_width,
//                         &[("N", " Scroll Dn"), ("S", " Save Text"), ("^Y", " Copy Txt"), ("", ""), ("", ""), ("", "")],
//                         ui_bg, menu_key_fg, menu_text_fg)?;
//                 }
//             }
//             // MenuState::EmailReader => {
