    ...
    *current_theme = editor.current_theme.clone();
    ...
    match builder.multipart(multipart) {
        Ok(email_msg) => {
            ...
            if is_microsoft {
                // GRAPH API BYPASS
                let token = match crate::net::get_oauth_access_token(
                    ...
                    Ok(t) => t,
                    Err(e) => return Some(format!("Graph Auth Error: {}", e)),
                };

                let mime_bytes = email_msg.formatted();
                let b64_mime = base64::prelude::BASE64_STANDARD.encode(&mime_bytes);
                ...
                match res {
                    ...
                    Ok(r) => Some(format!("Graph API Error: {}", r.text().unwrap_or_default())),
                ...
                if account.refresh_token.is_some() { mailer = mailer.authentication(vec![Mechanism::Xoauth2]); }
